...
class Vartools extends CI_Model implements CoreInterface{
     ...
     * @function    Get_ScadaWsData
     * @param       int d_device = id del dispositivo
     * @return      array
     *
     *   Algoritmo que genera variables en base a la fecha y el valor
     *     por ejemplo si var1 tiene 10 datos verifica si esos 10 datos estan en la misma fecha x
    ****/
    public function Get_ScadaWsData($id_device ) : array {

        $scadaWs    = $this->tools_devices->get_scada_data($id_device);

        $conf       = $this->tools_devices->get_scada_off_config()->date;
        ...
        foreach ($scadaWs as $d){
            ...
            $data    = json_decode($d->data)->data;
            $date    = new DateTime($d->date);

            foreach ($data as $varp ){
                if(!array_key_exists($varp->name , $variables)){

                    $variables[$varp->name] = new stdClass();
                    $variables[$varp->name]->values = array();
                }

                if($conf->delay == true)
                    $variables[$varp->name]->values[$date->format("Y-m-d h:m")][] = $varp->value;
                else
                    $variables[$varp->name]->values[$date->format("Y-m-d")][] = $varp->value;
