...
class Permissions extends CI_Model implements CoreInterface
{
    ...
    protected $admin_perm_id    = 1;


    protected $meta_perm        = null ;

    protected $ptable           = "rols";
    ...
    public function __construct()
    {
        ...
        $this->load->database();
        $this->ptable = $this->db->dbprefix($this->ptable);
    }



    private function get_metaPerm($id , $user , $name ){

        $this->meta_perm                        = new stdClass();
        $this->meta_perm->create_by             = new stdClass();

        $this->meta_perm->create_by->id         = $id ;
        $this->meta_perm->create_by->user       = $user;
        $this->meta_perm->create_by->name       = $name;

        $date   = new DateTime("now");
        $this->meta_perm->create_by->date       = $date->format("y-M-d h:m:s");
        $this->meta_perm->enable                = 1;

        return json_encode($this->meta_perm);
    ...
    public function _javascript()
    {
        ...
        return [
            ...
            array(
                "type"          => "text/babel" ,
                "location"      => "header" ,
                "script"        => site_url() . 'content/assets/apps/permission/render.jsx',
                "systemjs"      => false
            )
        ];
    }
    ...
    public function  exist_perm($name){

        $query = "SELECT count(*) as 'count' FROM $this->ptable as t WHERE t.name like ?";
        $result = $this->db
                        ->query($query , [$name])
                        ->result()[0]->count ??  1;
        return $result;
    ...
    {
         $name_perm         = strtolower($this->input->post("name"));

         if(empty($name_perm))
             return  json_encode([
                 "status"        => false ,
                 "msj"           => "Nombre de permiso vacio. !!"
             ]);
        ...
        if($id != $this->admin_perm_id)
            return  json_encode([
                "status"        => false ,
                "msj"           => "Usted no tiene privilegios administrativos para crear permisos"
            ]);


        //obtenemos el formato de los metadatos para el privilegio
        $metas      = $this->get_metaPerm($id , $user , $name);


        //verificamos los privilegios si existe un duplicado
        $result     = $this->exist_perm($name_perm);


        if($result >= 1)
            return  json_encode([
                "status"        => false ,
                "msj"           => "Este nombre de permiso ya existe, favor cambiar ese parametro"
            ]);



        $this->db->insert($this->ptable , [
            "name"      => $name_perm,
            "meta"      => $metas
        ]);
