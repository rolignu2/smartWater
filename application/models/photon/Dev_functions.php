...
class Dev_functions extends CI_Model implements CoreInterface
{
    ...
    public function __constructor(){
            ...
            $this->load->database();
            $this->db->dbprefix("functions");

            for($i = 0 ; $i < sizeof($this->querys) ; $i++  ){
                $this->querys[$i] = str_replace("FUNC_TABLE" , $this->table , $this->querys[$i]);
            }
    ...
    public function _render($params = NULL)
    {
        ...
        return $this->load->view("photon/function_render" , [
            "render" => $id !== 0 ? true : false ,
            "id"     => $id
        ] , TRUE );
