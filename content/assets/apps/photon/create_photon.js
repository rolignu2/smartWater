...
PhotonInsert = new React.createClass({
    ...
    render_table : function () {
          ...
          var data_Set = $.map(this.state.devices.data , function (a,b) {
              ...
              photon_.get_devices(a.id , null  ,   (r)=>{
                   try {
                       r = JSON.parse(r);
                       r = r[0];

                       device_detect += " (" + r.particle_id + ")<b>[" + r.name +"]</b>";
                       device_detect += ",";

                       $("#warn-message").css({display : "block"}).html(
                                    "El sistema ha detectado que el/los dispositivo/s"
                                    + device_detect + " se estan utilizando en otros paquetes y/o proyectos."
                                    + "<br><br> <center><b>Puede que halla conflicto al momento de configurar un dispositivo agregado anteriormente </b></center>" );
                   }catch(e){}
              });
                        ...
                        <form id="form-request" method="post" className="form-horizontal">
                            <div className="form-body">
                                ...
                                <div className="form-actions">
                                    <div className="row">
                                        {this.render_table()}
                                        <div id="warn-message" style={ { display : "none" } } className="alert alert-warning" > </div>
                                    </div>
